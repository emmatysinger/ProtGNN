...
# USE RANDOM FOLD FOR NOW or FULL GRAPH
def random_fold(df, fold_seed, frac):
    ...
    for i in df.relation.unique():
        ...
        df_train = pd.concat([df_train, train]) #df_train.append(train)
        df_valid = pd.concat([df_valid, val]) #df_valid.append(val)
        df_test = pd.concat([df_test, test]) #df_test.append(test)        
...
def get_wandb_log_dict(auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc, mode):
    ...
    relations = [('protein', 'molfunc_protein', 'molecular_function'),
                  ('molecular_function', 'molfunc_protein', 'protein')]
    ...
    name_mapping = {('protein', 'molfunc_protein', 'molecular_function'):' has Molecular Function ',
                  ('molecular_function', 'molfunc_protein', 'protein'): ' is Molecular Function of '}
...
def molfunc_centric_evaluation(df, df_train, df_valid, df_test, data_path, G, model, device, molfunc_ids = None, relation = None, weight_bias_track = False, wandb = None, show_plot = False, verbose = False, return_raw = False, simulate_random = True, only_prediction = False):
    ...
    pmf_etypes = [('protein', 'molfunc_protein', 'molecular_function')]
    ...
    def get_scores_molfunc(rel, molfunc_ids):
        ...
        for molfunc_id in tqdm(molfunc_ids):
            ...
            candidate_pos = df_rel_pmf[df_rel_pmf.x_idx == molfunc_id][['x_idx', 'y_idx']]
            candidate_pos_train = df_rel_pmf_train[df_rel_pmf_train.x_idx == molfunc_id]
            prot_pos = candidate_pos.y_idx.values
    ...
    if molfunc_ids is None:
        # downstream evaluate all test set molfuncs
        ...
        for rel_type in molfunc_rel_types:
            ...
            if weight_bias_track:
                temp_d.update({"molfunc_centric_evaluation_" + rel_type: wandb.Table(data=org_out,
                                    columns = ["metric_id", "metric", "mean", "std"])
                              })
        ...
        # downstream evaluate a specified list of molfuncs
        temp_d, preds_all, labels_all, metrics_all = {}, {}, {}, {}
